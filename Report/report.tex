...
\par
The second of the remaining tasks is completing our self-specification of \texttt{atoi} and comparing its result to \texttt{w0}'s symbolic value using the same input. Our specification of \texttt{atoi} currently takes as input a memory \texttt{mem}, string address \texttt{p}, index of digit start \texttt{j}, and total number of digits \texttt{k}, and then compute the symbolic atoi result of the number denoted by \texttt{mem[p+j]} to \texttt{mem[p+k-1]}. In the future, we would additionally want to compare those results with the contents of \texttt{w0} in the postcondition invariant (with an edge case of 0 digits being handled accordingly).
