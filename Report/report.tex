...
The \textbf{atoi} function takes a given input string, and converts the "initial portion" of it into a corresponding 32-bit integer. The initial portion of a string starts from the beginning of the string and ends after the first numeric sequence in the string. Save for a "+" or "-" to indicate the sign of the numeric sequence in the string, only white space can intervene between the beginning of the string and the first numeric sequence in order to produce a "valid" output. If this rule is violated, we call the input "ill-formed", and \textbf{atoi} will return \textbf{0}. Figure 1 shows a control flow graph for the disassembly of \textbf{atoi}.
\begin{figure*}[t]
	\centering
...
Throughout the course of our work on this project, we encountered a few challenges that made proving the correctness of \textbf{atoi} more complicated than originally anticipated. TODO

\section{Breakdown of Contributions} \label{contributions}
\begin{itemize}
	\item Matthew Sheldon TODO
	\item Isabella Pereira TODO
	\item Jarrod Rogers helped with organizing the project and wrote the majority of this report.
	\item Naja-Lee Habboush TODO
	\item Brandon Wang made the control flow graph (Figure 1) and TODO
\end{itemize}
...
As a team we have learned a lot from working on the correctness for \textbf{atoi}. iEven though there is still more work to be done, we are proud of what we have accomplished and the work we have contributed towards advancing the state-of-the-art.
